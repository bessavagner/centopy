from centopy.settings import PROJECT
from centopy.settings import HEADER
...
class Window(tk.Tk):
    def __init__(self, app: MyApp):
        ...
        name_label = tk.Label(label_frame, text=PROJECT['name'])
        name_label.pack(side=tk.LEFT, padx=10, pady=10)

        # Create a label for the app's version
        version_label = tk.Label(label_frame, text=f"v{PROJECT['version']}")
